...
function showGeminiCliOAuthModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-title">🔐 Gemini CLI OAuth</div>
...
function renderGeminiCliTokens(tokens) {
    ...
    if (filteredTokens.length === 0) {
        ...
        const emptyHint = currentGeminiCliFilter === 'all' ? 'Use the OAuth button above to add a token' : 'Click "Total" above to view all';
        tokenList.innerHTML = `
    ...
    tokenList.innerHTML = filteredTokens.map((token, index) => {
        ...
        <div class="token-card ${!token.enable ? 'disabled' : ''}" id="geminicli-card-${escapeHtml(cardId)}">
            ...
            <div class="token-info">
                ...
                <div class="info-row ${hasProjectId ? '' : 'warning'}" title="${hasProjectId ? 'Project ID' : 'Project ID missing, click to fetch'}">
                    <span class="info-label">📁</span>
...
async function deleteGeminiCliToken(tokenId) {
    const confirmed = await showConfirm('This action cannot be undone. Delete this token?', '⚠️ DeleteConfirmation');
    if (!confirmed) return;
...
async function exportGeminiCliTokens() {
    const password = await showPasswordPrompt('Please enter the admin password to export Gemini CLI tokens');
    if (!password) return;
...
async function reloadGeminiCliTokens() {
    ...
    try {
        ...
        hideLoading();
        if (data.success) {
            showToast('Reload successful', 'success');
            loadGeminiCliTokens();
        } else {
    ...
    modal.innerHTML = `
        <div class="modal-content modal-lg">
            ...
            <div class="import-tabs">
                <button class="import-tab active" data-tab="file" onclick="switchGeminiCliImportTab('file')">📁 Upload File</button>
                <button class="import-tab" data-tab="json" onclick="switchGeminiCliImportTab('json')">📝 Import JSON</button>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="geminicliImportMode">
                    <option value="merge">Merge (keep existing items, add or update matches)</option>
                    <option value="replace">Replace (clear existing items and import new ones)</option>
                </select>
                <p style="font-size: 0.75rem; color: var(--text-light); margin-top: 0.25rem;">💡 Uses refresh_token for deduplication: merge updates records with the same refresh_token</p>
            </div>
...
async function submitGeminiCliImport() {
    ...
    if (geminicliImportTab === 'file') {
        if (!geminicliImportFile) {
            showToast('Please choose a JSON file to import', 'warning');
            return;
        }
        ...
        if (!rawText.trim()) {
            showToast('Please paste the JSON content', 'warning');
            return;
        }
    }
